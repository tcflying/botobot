...
*底层硬核到极致,上手简单到只剩「打字发消息」。不用敲命令、不用背 prompt、不用懂 agent。*
...
「最软是数据」这一层本身也不是一团乱麻,而是一座按**写入门槛 × schema 严谨度**分级的**湖仓(Lakehouse)** —— 越往上越权威、约束越强、越当真理用;越往下门槛越低、越随手、越待提炼。这正是数据工程里「数据湖 → 数仓集市 → 主数据」的分层,被原样搬进了 agent 的知识体系:

| 层 | 湖仓类比 | 写入门槛 | schema | 谁来读 / 怎么读 |
|----|---------|---------|--------|----------------|
| **`book`** | 主数据 / 法规库 | 极高(严谨、权威) | **schema-on-write**,强约束 | 当**真理**引用 —— `read(book://#节点)` 翻开即权威,带 citation |
| **`skill`** | 数仓集市 | 高(已整理成操作手册) | 强结构(步骤化) | 当**流程**执行 —— 按 SOP 一步步照做 |
| **`memory`** | 特征表 / 缓存 | 中(一文件一事实 + frontmatter) | 半结构(有元数据) | **召回时按相关性** —— cosine + 实体扩散,带可信度分档 |
| **`docs`**(新) | 数据湖 | 极低(随手扔) | **schema-on-read** | **需要时翻、待提炼** —— 原始素材,后续可被巩固成上层 |

> 关键洞察:**知识不是一开始就该结构化的**。让它先以最低门槛「随手扔进数据湖(docs)」,再由记忆的巩固 pass、人工整理或 agent 自己**逐级提纯**——docs 沉淀成 memory,memory 固化成 skill,skill 升华成 book。**写入门槛从底到顶递增,权威性也从底到顶递增**,和湖仓「ETL 向上游精炼」完全同构。这让 agent 的知识既能**无摩擦地积累**(底层随便扔),又能**有纪律地被信任**(顶层强约束)。

> 一句话:**botobot 把「变化速率」当成第一性的分层维度**。硬的用 Rust 类型沉到底,软的用派生浮到顶,最软的化成数据在 `.bot/` 里流动 —— 而这层数据自身又是一座从「数据湖」精炼到「主数据」的知识湖仓。这让它既稳(地基不晃)又活(表皮可换),还能**越用越聪明**(知识逐级提纯)。
