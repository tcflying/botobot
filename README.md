...
2. **抄算法+协议,不 fork 身体** —— 借鉴 Codex / oh-my-pi 的算法与协议,自己用 Rust 实现,不继承整个 crate 堆。
3. **纯 Rust 一致性** —— 消灭 C 依赖,从神经网络到 shell,换单二进制与跨平台一致。
4. **只做必要的抽象** —— `Policy`/`Sandbox`/`ToolLookup` 必需时才上 trait,Driver 单一规范实现不泛化。
5. **观察驱动取代预付** —— 不预付未触发的能力,等真实痛点出现再重构;留接缝,不留半成品。
6. **eval 先行** —— 改记忆打分先过 eval 套件,基线门写进 CI 纪律,防止「优化」变退化。
